...
class BookModel
{
    ...
    {
        $addBookQry = $this->conn->prepare("INSERT INTO books (book_name, image, genre, author, edition, description, owner_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $addBookQry->bind_param("ssssisi", $bName, $bookDest, $bGenre, $bAuthor, $edition, $description, $ownerId);
        $addBookQry->execute();
        if ($addBookQry) {
            return true;
        } else {
